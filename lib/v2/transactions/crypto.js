...
function getBytes(transaction) {
	...
	bb.writeByte(transaction.network); // skelpy = 0x2D, testnet = 0x50
...
function verify(transaction, network) {
	network = network || networks.skelpy;
...
function getKeys(secret, network) {
	var ecpair = ECPair.fromSeed(secret, network || networks.skelpy);
