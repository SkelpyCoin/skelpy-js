...
describe("vote.js", function () {
  ...
  describe("#createVote", function () {
    var createVote = vote.createVote,
      vt = null,
      publicKey = skelpy.crypto.getKeys("secret").publicKey,
    ...
    it("should create vote from ecpair", function () {
      ...
      var secondSecretKey = skelpy.ECPair.fromSeed("second secret");
    ...
    it("should create vote from wif", function () {
      var secretKey = skelpy.ECPair.fromWIF("SB3iDxYmKgjkhfDZSKgLaBrp3Ynzd3yd3ZZF2ujVBK7vLpv6hWKK", NETWORKS.skelpy);
      secretKey.publicKey = secretKey.getPublicKeyBuffer().toString("hex");
    ...
    it("should be deserialised correctly", function () {
      var deserialisedTx = skelpy.crypto.fromBytes(skelpy.crypto.getBytes(vt).toString("hex"));
    ...
    describe("returned vote", function () {
      ...
      it("should have recipientId string equal to sender", function () {
        (vt).should.have.property("recipientId").and.be.type("string").and.equal(skelpy.crypto.getAddress(publicKey))
      });
      ...
      it("should be signed correctly", function () {
        var result = skelpy.crypto.verify(vt);
        (result).should.be.ok;
      });

      it("should be second signed correctly", function () {
        var result = skelpy.crypto.verifySecondSignature(vt, skelpy.crypto.getKeys("second secret").publicKey);
        (result).should.be.ok;
      });
